...
| `../gt4py/src/gt4py/next/program_processors/runners/dace/workflow/translation.py` | DaCe translation step — has `symbolic_domain_sizes` field in `DaCeTranslator`; `_preprocess_program` calls `apply_common_transforms` (which runs structured passes under `USE_STRUCTURED_BACKEND=1`) with `symbolic_domain_sizes`; `__call__` remaps `arg_types` via `StructuredTypeRemapper._cartesian_remapped_type` so DaCe bindings match the structured SDFG |
| `../gt4py/src/gt4py/next/program_processors/runners/dace/lowering/gtir_to_sdfg_concat_where.py` | DaCe `concat_where` lowering — **known limitation**: `translate_concat_where` raises `NotImplementedError` for `ts.TupleType` output (multi-output non-split stencils like `compute_avg_vn_and_graddiv_vn_and_vt`) |
| `../gt4py/src/gt4py/next/program_processors/runners/dace/lowering/gtir_to_sdfg_primitives.py` | DaCe GTIR→SDFG primitives — **known limitation**: `_parse_fieldop_arg` returns `None` for sparse-local fields like `e_bln_c_s` (`Field[CellDim, C2EDim]`) after structured remapping, causing `AttributeError: 'NoneType' has no attribute 'get_local_view'` |
...
All tested with `USE_STRUCTURED_BACKEND=1 PYTHONOPTIMIZE=1`. **gtfn_cpu** = original working backend; **dace_cpu** = DaCe backend (new, see §DaCe Backend Integration below):

| Stencil | Connectivities | gtfn_cpu | dace_cpu |
|---|---|---|---|
| `apply_divergence_damping_and_update_vn` | E2C, E2C2EO | ✅ | ✅ |
| `compute_avg_vn_and_graddiv_vn_and_vt` | E2C2EO, E2C2E | ✅ | ✅ |
| `add_interpolated_horizontal_advection_of_w` | C2E (sparse `e_bln_c_s`) | ✅ | ❌ get_local_view None |
| `add_extra_diffusion_for_w_con_approaching_cfl` | C2E2CO | ✅ | ✅ |
| `interpolate_to_cell_center` | C2E | ✅ | ✅ |
| `mo_math_divrot_rot_vertex_ri_dsl` | V2E | ✅ | ✅ |
| `mo_icon_interpolation_scalar_cells2verts_scalar_ri_dsl` | V2C | ✅ | ✅ |
| `apply_diffusion_to_vn` | E2C2V, `start_2nd_nudge_line_idx_e` threshold | ✅ | ✅ (all 3 variants) |
| `compute_advection_in_horizontal_momentum` | E2C, E2V, E2C2EO, C2E, V2E | ✅ | ✅ (all 4 variants) |
...
### Overview
The DaCe backend (`--backend=dace_cpu`) runs the structured backend passes inside DaCe's `_preprocess_program` (via `apply_common_transforms` with `USE_STRUCTURED_BACKEND=1`) and then lowers the structured GTIR to a DaCe SDFG. This is separate from the GTFN path and requires special wiring.

### gt4py branch
Use the **`grafrap_dace`** branch of `../gt4py`. This branch has all structured-backend-aware DaCe changes.

### How it works (key wiring)

**`cartesian_interceptor.py` — `_get_or_compile`**: detects `DaCeBackendFactory` by `isinstance(self._backend_factory, type) and issubclass(...)`. For DaCe, calls `DaCeBackendFactory(gpu=False, cached=True, auto_optimize=True, ..., otf_workflow__bare_translation__apply_common_transform=True, otf_workflow__bare_translation__symbolic_domain_sizes=sds)`. This is required because `DaCeBackendFactory.auto_optimize` is a `factory.Trait` and cannot be accessed via `SelfAttribute` unless explicitly set — calling with just `symbolic_domain_sizes` fails with `AttributeError: auto_optimize is unknown`. DaCe compilation caching: `otf_workflow__cached_translation=False` (to avoid reusing wrong SDFG for different `horizontal_start` values); the in-process `_compiled_cache` provides per-`(horizontal_start, thresholds)` caching instead.

**`translation.py` — `DaCeTranslator`**: has `apply_common_transform: bool` and `symbolic_domain_sizes: dict | None` fields. When `apply_common_transform=True`, `_preprocess_program` calls `apply_common_transforms(..., symbolic_domain_sizes=self.symbolic_domain_sizes)`. Under `USE_STRUCTURED_BACKEND=1`, this runs `CartesianDomainAndTypeRemapper` + `CartesianReductionUnroller` before the DaCe SDFG lowering.

**`translation.py` — `DaCeTranslator.__call__`**: after SDFG generation, remaps `arg_types` from unstructured to structured via `StructuredTypeRemapper._cartesian_remapped_type(t)`. This is necessary because `inp.args.args` carries the original unstructured types (e.g. `Field[Edge, K]`) while the SDFG has structured shapes (e.g. `Field[IDim, JDim, Kolor, K]`). Without this remap the bindings step (`bindings.py`) raises `ValueError: zip() argument 2 is longer than argument 1`.

**`DaCeTranslationStepFactory`**: has defaults for all required `DaCeTranslator` fields (`auto_optimize_args=None`, `async_sdfg_call=False`, `unstructured_horizontal_has_unit_stride=False`, `use_metrics=True`, `symbolic_domain_sizes=None`). These allow calling `DaCeBackendFactory(...)` with only a subset of kwargs; `make_dace_backend` always overrides them explicitly.

**`conftest.py` (dycore) / `stencil_tests.py`**: detect `_is_dace` from `backend_like["backend_factory"].__name__` containing "dace", then set `_chosen_factory = DaCeBackendFactory` (the class, not a partial — the class identity check in `_get_or_compile` requires a type).
...
1. **`concat_where` with tuple output** (resolved): `gtir_to_sdfg_concat_where.py` was extended with two handlers for `ts.TupleType` output: (a) fast path for `make_tuple` branches — splits element-wise with correct Python-tuple domain annotation; (b) general path for `as_fieldop` branches — visits both branches to get tuples of `FieldopData`, then lowers each element via `_translate_concat_where_from_data`. The key bug was that the fast path set `elem_node.annex.domain = raw_domain` (full tuple) instead of `raw_domain[i]`.

2. **`get_local_view` on `None`** (resolved): `_parse_fieldop_arg` now recovers when `translate_symbol_ref` returns `None` (NEVER domain) for sparse-local fields after structured remapping by calling `_get_data_nodes` directly.

### Running with DaCe

```bash
export USE_STRUCTURED_BACKEND=1 && export PYTHONOPTIMIZE=1 && export OMP_NUM_THREADS=4
rm -rf .gt4py_cache ~/.gt4py_cache 2>/dev/null
pytest -q <test_file> --backend=dace_cpu --grid ../grid-generator/parallelogram_grid.nc:5 --maxfail=1 -s > test_out.txt 2>&1; tail -30 test_out.txt
```

`OMP_NUM_THREADS=4` limits DaCe's OpenMP runtime parallelism to 4 cores. The commands in `commands_stencils.txt` already include this.
...
# Use --backend=gtfn_cpu for the original structured backend (default):
...
# Use --backend=dace_cpu for the DaCe backend (requires grafrap_dace gt4py branch):
pytest -q <test_file> --backend=dace_cpu --grid ../grid-generator/parallelogram_grid.nc:<levels> --maxfail=2 -s > test_out.txt 2>&1; tail -40 test_out.txt
