...
def test_apply_diffusion_to_vn_cartesian(backend="gtfn_cpu"):
    ...
    # Build Map & Mask boundary natively from the structured layout!
    e2c2v_np, pn_v1_s, pn_v2_s = build_e2c2v_and_pn(index_map, n_edges, pn_v1_np, pn_v2_np)
    
    is_interior_edge_np = np.zeros(n_edges, dtype=bool)
    for i in range(ni):
        for j in range(nj):
            for k in range(3):
                ...
                if e >= 0:
                    if (4 <= i < ni - 4) and (4 <= j < nj - 4):
                        is_interior_edge_np[e] = True

    # Pad safe vertices for unstructured reference evaluate
    u_vert_safe = np.vstack([u_vert_np, np.zeros((1, num_levels))])
    v_vert_safe = np.vstack([v_vert_np, np.zeros((1, num_levels))])

    expected_vn = apply_diffusion_to_vn_numpy(
        e2c2v_np, u_vert_safe, v_vert_safe, pn_v1_np, pn_v2_np, z_nabla2_e_np, inv_vv_len_np, inv_pe_len_np,
        area_edge_np, kh_smag_e_np, diff_multfac_vn_np, nudgecoeff_e_np, vn_np.copy(), 
        nudgezone_diff, fac_bdydiff_v, is_interior_edge_np
    )
